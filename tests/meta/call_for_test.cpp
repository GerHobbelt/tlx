...
{
    tlx::call_foreach([&os](auto a) { os << a << '\n'; }, args...);
...
void test_call_foreach_with_index_run(std::ostream& os, const Args&... args)
{
    ...
    tlx::call_foreach_tuple_with_index(SimpleFunctorWithIndex(os),
                                       std::make_tuple(args...));
}
...
static void test_call_for_range()
{
    ...
    tlx::call_for_range<6>([&](size_t index) { oss << index << ' '; });

    tlx::call_for_range<4, 8>([&](auto index) { oss << index << ' '; });
