...
static void test_merge_combine()
{
    ...
    std::vector<Pair> vec1 = {{1, 10}, {2, 10}, {5, 10}, {7, 10}, {9, 10}};
    std::vector<Pair> vec2 = {{0, 5}, {2, 5}, {6, 5}, {7, 5}, {11, 5}};
    ...
    tlx::merge_combine(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(),
                       std::back_inserter(out), comp,
                       [](const Pair& a, const Pair& b) {
                           return Pair(a.first, a.second + b.second);
                       });

    std::vector<Pair> result = {{0, 5}, {1, 10}, {2, 15}, {5, 10},
                                {6, 5}, {7, 15}, {9, 10}, {11, 5}};
...
{
    {
        ...
        int init = 1;
        const auto res1 =
            tlx::exclusive_scan(vec1.begin(), vec1.end(), vec2.begin(), init);
        LOG0 << vec2.end() - res1;
        LOG0 << vec2.front();
...

    {
        ...
        const auto res1 = tlx::exclusive_scan(vec1.begin(), vec1.end(),
                                              vec2.begin(), init, binary_op);
        die_unless(res1 == vec2.begin() + 4);
        die_unless(std::equal(vec_res.begin(), vec_res.end(), vec2.begin()));
    }
}
